if Meteor.isServer
  Meteor.startup () ->
    ...
    ScheduleJobPipeline [[{processorType: 'Processor', settings: {name: 'pipeline'}}],
          [{processorType: 'Processor', settings: {name: 'pipeline'}}, {processorType: 'Processor', settings: {name: 'pipeline'}}]
          [{processorType: 'Processor', settings: {name: 'pipeline'}}]]
    ...
    Tinytest.add 'Pipelines - ScheduleJobPipeline - all jobs scheduled', (test) ->
      jobs = JobQueue.find {settings: {name: 'pipeline'}}
      test.isTrue jobs.count() == 4
